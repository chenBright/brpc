...
#define BAIDU_OBJECT_POOL_GET(CTOR_ARGS)                                \
        ...
        if (_cur_block && _cur_block->nitem < BLOCK_NITEM) {            \
            ...
            BUTIL_ASAN_POISON_MEMORY_REGION(obj, sizeof(T));            \
            ++_cur_block->nitem;                                        \
            return obj;                                                 \
        }                                                               \
...
#undef BAIDU_OBJECT_POOL_GET
        ...
        inline int return_object(T* ptr) {
            ...
            // Marks the memory as unaddressable.
            BUTIL_ASAN_POISON_MEMORY_REGION(ptr, sizeof(T));
            // TODO. Refer to ASan to implement the quarantine mechanism.
#endif
    ...
    inline T* get_object(Args&&... args) {
        ...
        if (BAIDU_LIKELY(lp != NULL)) {
            ...
            // Marks the memory as addressable.
        ...
        for (size_t i = 0; i < ngroup; ++i) {
            ...
            for (size_t j = 0; j < nblock; ++j) {
                ...
                for (size_t k = 0; k < b->nitem; ++k) {
                    ...
                    BUTIL_ASAN_UNPOISON_MEMORY_REGION(obj, sizeof(T));
                    obj->~T();
                }
                delete b;
            }
