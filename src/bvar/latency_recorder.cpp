...
// LatencyRecorder is made of. Hosts keeping their data in a shared combiner are
// sampled through that carrier, so sampling reads valid memory even if the host
// is destructed before the sampler is recycled. PassiveStatus keeps its data in
// a user callback instead, hence it is still sampled through the host pointer.
