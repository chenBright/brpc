...
class CommonVariable: public Variable {
public:
    typedef ReducerSampler<CommonVariable, T, Op, InvOp> sampler_type;
    typedef SeriesSamplerImpl<CommonVariable, T, Op> series_sampler_type;

    CommonVariable() = default;
    template<typename U = T, typename std::enable_if<
        std::is_constructible<Counter, U>::value, bool>::type = false>
    CommonVariable(U default_value) : _counter(default_value) {}
    template<typename U = T, typename std::enable_if<
        !std::is_constructible<Counter, U>::value, bool>::type = false>
    CommonVariable(U) {}

    DISALLOW_COPY_AND_MOVE(CommonVariable);

    ~CommonVariable() override {
        hide();
        if (NULL!= _sampler) {
            _sampler->destroy();
        }
    ...
    CommonVariable& operator<<(T value) {
        _counter << value;
        return *this;
    }
...
class Adder<T, std::enable_if<std::is_constructible<babylon::GenericsConcurrentAdder<T>>::value>>
    : public detail::CommonVariable<T, babylon::GenericsConcurrentAdder<T>,
                                    detail::AddTo<T>, detail::MinusFrom<T>> {
public:
    typedef  T value_type;
private:
    typedef detail::CommonVariable<T, babylon::GenericsConcurrentAdder<T>,
...
public:
    ...
    Adder& operator<<(value_type value) {
        Base::operator<<(value);
        return *this;
    }
};
...
class Maxer<T, std::enable_if<std::is_constructible<detail::ConcurrentMaxer<T>>::value>>
    : public detail::CommonVariable<
        T, detail::ConcurrentMaxer<T>, detail::MaxTo<T>, detail::VoidOp> {
public:
    typedef T value_type;
private:
    typedef detail::CommonVariable<
        T, detail::ConcurrentMaxer<T>, detail::MaxTo<value_type>, detail::VoidOp> Base;
public:
    ...
    Maxer& operator<<(value_type value) {
        Base::operator<<(value);
        return *this;
    }
...
class Miner<T, std::enable_if<std::is_constructible<babylon::GenericsConcurrentMiner<T>>::value>>
    : public detail::CommonVariable<
        T, babylon::GenericsConcurrentMiner<T>, detail::MinTo<T>, detail::VoidOp> {
public:
    typedef T value_type;
private:
    typedef detail::CommonVariable<
        T, babylon::GenericsConcurrentMiner<T>, detail::MinTo<value_type>, detail::VoidOp> Base;
public:
    ...
    Miner& operator<<(value_type value) {
        Base::operator<<(value);
        return *this;
    }
};
