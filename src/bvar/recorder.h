...
public:
    ...
    sampler_type* get_sampler() {
        if (nullptr == _sampler) {
            _sampler = new sampler_type(this);
            // Remember the name for diagnostics: hide() clears it before
            // destroy() is called. It may still be empty here, see the
            // non-babylon IntRecorder for details.
            _sampler->set_debug_name(diagnostic_name());
            _sampler->schedule();
        }
        return _sampler;
...
protected:
    int expose_impl(const butil::StringPiece& prefix,
        ...
        if (rc == 0 && nullptr != _sampler) {
            // The sampler may predate the name, see get_sampler().
            _sampler->set_debug_name(diagnostic_name());
        }
        return rc;
    }
