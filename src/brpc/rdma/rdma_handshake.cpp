...
// Dynamically loaded in rdma_helper.cpp; NULL when libibverbs predates ECE.
extern int (*IbvQueryEce)(ibv_qp*, ibv_ece*);

DEFINE_bool(rdma_ece, false, "Enable end-to-end ECE (Enhanced Connection Establishment) "
                             "negotiation in the RDMA v3 handshake. Requires rdma-core >= v35 "
                             "(IBVERBS_1.10) on both peers; automatically degrades to no-ECE "
                             "when the peer, the local libibverbs, or set_ece does not support "
                             "it. Acts as a kill switch (default off).");
...
void FillLocalRdmaHello(const RdmaEndpoint* ep, RdmaHello* msg) {
    ...
    if (FLAGS_rdma_ece && ep->_outgoing_ece.has_value()) {
        RdmaEce* pe = msg->mutable_ece();
        pe->set_vendor_id(ep->_outgoing_ece->vendor_id);
        pe->set_options(ep->_outgoing_ece->options);
        pe->set_comp_mask(ep->_outgoing_ece->comp_mask);
    }
}
