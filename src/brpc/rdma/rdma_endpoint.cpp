...
void RdmaConnect::StartConnect(const Socket* socket,
    ...
    if (!IsRdmaAvailable()) {
        rdma_transport->_rdma_ep->_state.store(
            RdmaEndpoint::FALLBACK_TCP, butil::memory_order_relaxed);
...
void* RdmaEndpoint::ProcessHandshakeAtClient(void* arg) {
    ...
    bool rdma_off = rdma_transport->_rdma_state != RdmaTransport::RDMA_OFF;
    uint32_t flags = rdma_off ? HELLO_ACK_RDMA_OK : 0;
