...
#define LoadSymbolOptional(handle, func, symbol) \
    *(void**)(&func) = dlsym(handle, symbol); \
    LOG_IF(INFO, !func) << "Optional symbol not found (feature disabled): " << symbol;
...
static int ReadRdmaDynamicLib() {
    ...
    // ECE APIs were introduced in rdma-core v35 (symbol version
    // IBVERBS_1.10). Load them optionally so that running against an older
    // libibverbs keeps RDMA working (ECE simply stays disabled) instead of
