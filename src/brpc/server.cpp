...
int Server::StartInternal(const butil::EndPoint& endpoint,
    ...
    const ServerOptions& eff_opt = opt ? *opt : default_opt;

    if (!eff_opt.h2_settings.IsValid(true/*log_error*/)) {
        LOG(ERROR) << "Invalid h2_settings";
        return -1;
    }

    if (eff_opt.bthread_tag < BTHREAD_TAG_DEFAULT ||
        eff_opt.bthread_tag >= FLAGS_task_group_ntags) {
        LOG(ERROR) << "Fail to set tag " << eff_opt.bthread_tag
                   << ", tag range is [" << BTHREAD_TAG_DEFAULT << ":"
                   << FLAGS_task_group_ntags << ")";
        return -1;
    }
    int ret = TransportFactory::ContextInitOrDie(eff_opt.socket_mode, true, &eff_opt);
    if (ret != 0) {
        LOG(ERROR) << "Fail to initialize transport context for server, ret=" << ret;
        return -1;
    }

    copy_and_fill_server_options(_options, eff_opt);
