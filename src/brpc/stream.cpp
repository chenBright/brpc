...
int Stream::AppendIfNotFull(const butil::IOBuf &data,
    ...
    // taking _connect_mutex. `_connected` is a one-way transition published by
    // SetConnected() *after* flushing pending writes, so ordering is preserved
    ...
    // will flush it.
    // Double-check Failed() under the lock to handle racing with
    // SetConnected()/OnFailed(). If Failed() is still false here, OnFailed()
    // has not bumped the version yet, so its _pending_writes.clear() (which
    // also takes _connect_mutex) is ordered after this emplace_back and will
    // discard the buffered data; if it is true, we return without buffering.
    {
        BAIDU_SCOPED_LOCK(_connect_mutex);
        if (Failed()) {
            RollbackProduced(data_length);
        ...
        // Ordering holds: reaching here means we acquired _connect_mutex, which
        // SetConnected() releases only after it has flushed all pending writes
        // (enqueued into the host socket) and published _connected=true. Hence
        // this direct write is necessarily enqueued after those pending writes.
    }
...
void Stream::SetConnected(const StreamSettings* remote_settings) {
    ...
    Socket* const host_socket = _host_socket;
    CHECK(host_socket != NULL);
    if (remote_settings != NULL) {
    ...
    RPC_VLOG << "stream=" << id() << " is connected to stream_id="
             << _remote_settings.stream_id() << " at host_socket=" << *host_socket;
    ...
    for (size_t i = 0; i < pending.size(); ++i) {
        ...
        if (FLAGS_socket_max_streams_unconsumed_bytes > 0) {
            host_socket->_total_streams_unconsumed_size += len;
        }
    }
