...
bool Compress(CompressCallback& callback,
    ...
    butil::IOBufAsZeroCopyOutputStream stream(&callback.Buffer());
    google::protobuf::io::GzipOutputStream gzip(&stream, options);
    if (!callback.Convert(&gzip)) {
        if (NULL != gzip.ZlibErrorMessage()) {
            callback.append_error(gzip.ZlibErrorMessage());
        }
        return false;
    }
    return true;
...
bool Decompress(DecompressCallback& callback, google::protobuf::io::GzipInputStream::Format format) {
    butil::IOBufAsZeroCopyInputStream stream(callback.Buffer());
    google::protobuf::io::GzipInputStream gzip(&stream, format);
    if (!callback.Convert(&gzip)) {
        if (NULL != gzip.ZlibErrorMessage()) {
            callback.append_error(gzip.ZlibErrorMessage());
        }
...
bool GzipCompress(CompressCallback& callback) {
    return Compress(callback, google::protobuf::io::GzipOutputStream::GZIP);
}

bool GzipDecompress(DecompressCallback& callback) {
    return Decompress(callback, google::protobuf::io::GzipInputStream::GZIP);
}

bool ZlibCompress(CompressCallback& callback) {
    return Compress(callback, google::protobuf::io::GzipOutputStream::ZLIB);
}

bool ZlibDecompress(DecompressCallback& callback) {
    return Decompress(callback, google::protobuf::io::GzipInputStream::ZLIB);
}
...
static bool Compress(const google::protobuf::Message& msg, butil::IOBuf* buf,
    ...
    if (!msg.SerializeToZeroCopyStream(&gzip)) {
        LOG(WARNING) << "Fail to serialize input pb=" << msg.GetDescriptor()->full_name()
                     << ", format=" << Format2CStr(format)
                     << " : " << gzip.ZlibErrorMessage();
        return false;
    }
    return gzip.Close();
}
...
static bool Decompress(const butil::IOBuf& data, google::protobuf::Message* msg,
    ...
    if (!ParsePbFromZeroCopyStream(msg, &gzip)) {
        LOG(WARNING) << "Fail to parse input pb=" << msg->GetDescriptor()->full_name()
                     << ", format=" << Format2CStr(format)
                     << " : " << gzip.ZlibErrorMessage();
        return false;
    }
