...
- 完整列出所有应发布的合并 PR；文档、测试和构建类变更可归入 `Enhancements`；
- **变更条目**按 `Features`、`Bugfixes`、`Enhancements` 分类；同一分类内按功能主题排序，并且仅合并重复描述的同一 PR；
...
“Bug 修复”和“功能增强”列出变更，最后统一致谢。每项保留 `by @GitHub-ID (#PR-id)`，不写显式 PR URL；
公众号不需要逐条展开链接。参考结构：
...
- 保持与 GitHub Release 一致的三类分类和 PR 条目粒度；同一 GitHub ID 的不同功能或修复仍单独成条；
