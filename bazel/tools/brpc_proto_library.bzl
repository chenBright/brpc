...
#   - This macro removes brpc's own coupling to
#     `@rules_cc` / `@rules_proto` / protobuf's bazel macros, so the
#     three BUILD.bazel files in brpc do not need to change when those
#     macros' load paths shift between versions.
#   - It does NOT free us from the Bazel version requirement that
#     protobuf itself imposes: protobuf >= 34 calls
#     `ProtoInfo(option_deps = ..., extension_declarations = ...)` in
#     its own `proto_library` implementation, and those fields are
#     only accepted by Bazel >= 8. Since this macro still depends on
#     `@com_google_protobuf//:descriptor_proto` (which IS a
#     proto_library), upgrading protobuf still requires bumping Bazel
#     accordingly.
